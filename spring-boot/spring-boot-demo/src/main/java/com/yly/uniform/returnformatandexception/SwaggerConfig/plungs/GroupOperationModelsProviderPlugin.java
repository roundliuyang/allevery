...
public class GroupOperationModelsProviderPlugin implements OperationModelsProviderPlugin {
    ...
    public void apply(RequestMappingContext context) {
        ...
        context.getParameters().stream()
                .forEach(rmp -> {
                    Class[] groups = com.yly.uniform.returnformatandexception.SwaggerConfig.plungs.GroupOperationBuilderPlugin.getGroups(rmp, ResolvedMethodParameter::findAnnotation);
                    if (groups != null) {
                        ((Set<ModelContext>) ReflectionUtils.getField(CONTEXTS, context.operationModelsBuilder())).stream()
                                ...
                                .forEach(mc -> ReflectionUtils.setField(DOCUMENTATION_TYPE,
                                        mc,
                                        new com.yly.uniform.returnformatandexception.SwaggerConfig.plungs.GroupDocumentationType(mc.getDocumentationType(), groups)));
                    }
                });
        // 查找返回值group参数
        Class[] groups = com.yly.uniform.returnformatandexception.SwaggerConfig.plungs.GroupOperationBuilderPlugin.getGroups(context, RequestMappingContext::findAnnotation);
        if (groups != null) {
            ((Set<ModelContext>) ReflectionUtils.getField(CONTEXTS, context.operationModelsBuilder())).stream()
