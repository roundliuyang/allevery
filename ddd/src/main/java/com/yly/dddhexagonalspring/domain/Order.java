...
/**
 * 这是我们的聚合根。任何与我们的业务逻辑相关的东西都会通过这个类。此外，  Order负责将自身保持在正确的状态
 *  1.The order can only be created with the given ID and based on one Product – the constructor itself also inits the order with CREATED status
 *  2.Once the order is completed, changing OrderItems is impossible
 *  3.It's impossible to change the Order from outside the domain object, like with a setter
 */
public class Order {
    private UUID id;
